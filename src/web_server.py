...
from a MCC 128 DAQ HAT device for a single client.  It makes use of the Dash
...
_app.layout = html.Div([
    html.H1(
        children='MCC 128 DAQ HAT Web Server Example',
        id='exampleTitle'
    ),
    html.Div([
        html.Div(
        ...
        html.Div(
            ...
            ], style={'width': 320, 'box-sizing': 'border-box', 'padding': 12,
...
def update_timer_interval(acq_state, chart_data_json_str, chart_info_json_str,
    ...
    refresh_rate = 1000  # 1 second when idle or configured
    ...
    if acq_state == 'running':
        ...
        if chart_info['sample_count'] == chart_data['sample_count']:
            ...
            # Data is being processed, temporarily slow down the timer
            refresh_rate = 2000
...
def disable_channel_checkboxes(acq_state):
    ...
    """
    options = []
    for channel in range(MCC128_CHANNEL_COUNT):
...
def add_samples_to_data(samples_to_display, num_chans, chart_data, read_result):
    """
    Adds the samples read from the mcc128 hat device to the chart_data object
...
def update_strip_chart(chart_data_json_str, active_channels, input_range):
    """
    ...
    figure = {
        ...
        'layout': go.Layout(
            ...
            yaxis=dict(title='Voltage', range=yaxis_range, autorange=False),
